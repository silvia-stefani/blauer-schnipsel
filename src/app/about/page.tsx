    ...
    return <main className={styles.about}>
        ...
        <Grid cols={4}>
                ...
                <div className={styles.container}>
                    ...
                    {contacts.map((c) => {
                        ...
                        return <div key={c.id}>
                            ...
                            <a href={href} target='_blank'>{c.acf.value}</a>
                        </div>
                    })}
                    </div>
                </div>
                ...
                <div className={styles.container}>
                    {team.map((t) => {
                        ...
                        <div className={styles.member} key={t.id}>
                            ...
                            <div className={styles.role}>{t.acf.role[`role_${currentLocale}`]}</div>
                        </div>
                        )
                    })}
                </div>
            ...
            {allCols.map((col) => {
                ...
                return <div key={col.id} className={styles.column}>
                    <div className={styles.container}>
                        <h4 className={styles.title}>{col.acf.label[currentLocale]}</h4>
                        {cats.map((data) => {
                            const projects = allProjects.filter((ap) => ap.curriculum_category.some(id => id === data.id));
                            ...
                            return <div className={styles.group} key={data.id}>
                                <h5 className={styles.title}>{data.acf.label[currentLocale]}</h5>
                                <div className={styles.container}>

                                    {result.map((b) => {
                                        return <div key={b.id} className={styles.block}>
                                            <div className={styles.year}>{b.name}</div>
                                            {b.projects.map((p, i) => {
                                                return <div key={p.id} className={styles.text}>{p.acf.text[currentLocale]}</div>
                                            })}
                                        </div>
                                    })}
