        ...
        <div className={`${styles.menu} ${open ? styles.open : ''}`}>
            ...
            <nav className={styles.navbar}>
                {largeDevice && <ul className={styles.list}>
                    {menuItems.map((mi) => (
                        <li className={`${styles.item} ${mi.isDisabled ? styles.disabled : ''}`} key={mi.id}>
                            {mi.isDisabled && <span>{Hammer}</span>}
                            <Link aria-disabled={mi.isDisabled} className={styles.link} href={`/${mi.id}`}>{t(mi.labelKey)}</Link>
                        </li>
                    ))}
                </ul>}
            </nav>
